...
import * as tripleSolver from "./triple_solver";
...
app.delete("/group/:id", (req, res) => {
  ...
  admin.firestore().collection("groups")
      .doc(req.params.id).collection("preferences").get()
      .then((snapshot) => {
        if (snapshot.size === 0) {
          return 0;
        }
      ...
      .then(() => {
        return admin.firestore()
            .collection("groups").doc(req.params.id).delete()
            .then(() => {
...
const preferencesForGroup = (groupId: string): Promise<PreferenceRecord> => {
  return admin.firestore()
      .collection("groups").doc(groupId).collection("preferences").get()
      .then((snapshot) => {
        if (snapshot.size === 0) {
...
app.get("/group/:id/preferences", (req, res) => {
  preferencesForGroup(req.params.id).then((result) => {
    res.status(200).json({
      "preferences": Object.keys(result.student_prefs[0]),
    });
  })
      .catch((error) => {
        console.log(
            "Unable to get preferences for group " + req.params.id,
            error
        );
        res.status(500).json({error: error.message});
      });
...
app.get("/group/:id/raw_preferences", (req, res) => {
  preferencesForGroup(req.params.id).then((result) => {
    res.send(result);
      ...
      .catch((error) => {
        console.log(
            "Unable to get match result for group " + req.params.id,
            error
        );
        res.status(500).json({error: error.message});
      });
...
app.get("/group/:id/triples", (req, res) => {
  return preferencesForGroup(req.params.id).then((result) => {
    res.status(200).send(tripleSolver.solveForTriples(result));
  })
      .catch((error) => {
        console.log(
            "Unable to get match result for group " + req.params.id,
...
app.post("/group/:id/preference", (req, res) => {
  ...
  console.log("Got postData", postData);
  admin.firestore()
      .collection("groups").doc(req.params.id)
      .collection("preferences").doc(postData.me).set({
